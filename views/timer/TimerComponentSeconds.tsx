...
type LapProps = {
  interval: {timer: string};
  opacity: Animated.AnimatedInterpolation;
  scale: Animated.AnimatedInterpolation;
};

function Lap({interval, opacity, scale}: LapProps) {
  let time = moment.duration('PT' + interval.timer + 'S');
  return (
...
type TimersComponentProps = {
  ...
  laps: {timer: string; timerString: string}[];
};

function TimerComponentSeconds({
  setIndexTimerSelected,
  laps,
}: TimersComponentProps) {
...
export default TimerComponentSeconds;
